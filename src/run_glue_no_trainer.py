...
def parse_args():
    ...
    parser.add_argument(
        "--from_scratch",
        action="store_true",
        help="If passed, train from scratch.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1,
        help="Coefficient for skim loss during training.",
    )
    parser.add_argument(
        "--do_train",
        action="store_true",
...
def main():
    ...
    if args.task_name == "imdb":
        raw_datasets = load_dataset(
            'imdb', ignore_verifications=True,
        )        
    ...
    if args.from_scratch:
        if args.model_type == 'transkimer':
            ...
            config.skim_coefficient = args.skim_coefficient
            model = TranskimerRobertaForSequenceClassification.from_pretrained(
                args.model_name_or_path,
                from_tf=bool(".ckpt" in args.model_name_or_path),
                config=config,
                state_dict={},
    ...
    if args.task_name is not None and args.task_name != 'imdb':
